...
import { ipcFailure, ipcSuccess, type IPCResult } from '@shared/types/ipc'
...
export function registerIPCHandlers(useCase: WorktreeMainUseCase): void {
  ...
  ipcMain.handle(
    'worktree:status',
    (_event, params: { repoPath: string; worktreePath: string }) =>
  ...
  ipcMain.handle(
    ...
    (_event, params: { repoPath: string; branch: string }) =>
