...
| OperationProgress の domain 型設計 | 低 | **解決済み**: 独自型として `src/features/advanced-git-operations/di-tokens.ts:13` に `OperationProgress` を定義（`ObservableStoreUseCase<OperationProgress \| null>` で公開） |
