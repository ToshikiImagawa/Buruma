...
本 feature は Clean Architecture 4層構成（A-004）に従い、Webview 側は `src/features/application-foundation/`、Tauri Core (Rust) 側は `src-tauri/src/features/application_foundation/` に実装する。Webview 側の境界共有 domain 型は `src/domain/`（TypeScript）に配置し、Rust 側は feature ごとの `src-tauri/src/features/application_foundation/domain.rs` で serde 付き struct として整合させる（現行実装では共有の `src-tauri/src/domain/` ディレクトリは使用しない）。
...
| 層 | 責務 |
|:---|:---|
| **domain** | エンティティ（`src/domain/` から境界間共有） |
...
| 層 | 責務 |
|:---|:---|
| **domain** | エンティティ（`src-tauri/src/features/application_foundation/domain.rs` の serde 付き struct） |
...
| UseCase | application 層のステートレスな操作単位。Webview 側は `src/lib/usecase/` の型を継承する |
