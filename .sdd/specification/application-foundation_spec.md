...
本 feature は Clean Architecture 4層構成（A-004）に従い、Webview 側は `src/features/application-foundation/`、Tauri Core (Rust) 側は `src-tauri/src/features/application_foundation/` に実装する。domain 型は `src/domain/`（TypeScript）に配置し、Rust 側は `src-tauri/src/domain/` で serde 付き struct として整合させる。
