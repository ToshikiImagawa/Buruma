...
本 feature は Clean Architecture 4層構成（A-004）に従い、レンダラー側は `src/renderer/features/application-foundation/`、メインプロセス側は `src/main/features/application-foundation/` に実装する。domain 型は `src/shared/domain/` に配置しプロセス間で共有する。
