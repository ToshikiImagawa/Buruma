...
既存の IPC チャネル（77 command + 8 event）を以下のルールで Tauri command / event に 1:1 マッピングする。
...
Rust 側は慣用的に `Result<T, AppError>` を返し、Tauri は `Err(AppError)` を invoke の reject として JSON 化する。TypeScript 側は `src/shared/lib/invoke/commands.ts` の `invokeCommand<T>` ラッパー経由で呼び出し、以下のように `IPCResult<T>` 形式に変換する:

```typescript
// src/shared/lib/invoke/commands.ts
...
export async function invokeCommand<T>(
  ...
  try {
    ...
    const error: IPCError = typeof e === 'object' && e !== null && 'code' in e
      ? (e as IPCError)
...
Rust 側は `AppError` enum の `Serialize` 実装により、`{ code, message, detail? }` 形式の JSON としてシリアライズされ、上記の `IPCError` と整合する。
...
| ID | 要件 | 優先度 | 根拠 (PRD) |
|--------|------|------|------|
| FR-001 | 77 個の既存 invoke/handle チャネルを Tauri command に 1:1 マッピングする | 必須 | FR_M01 |
...
## 4.1. Commands マッピング表（77 個、Webview → Core `invoke`）
...
### 4.1.6. Claude Code Integration（14 個）
...
export class WorktreeDefaultRepository implements WorktreeRepository {
  ...
  async onChanged(cb: (event: WorktreeChangeEvent) => void): Promise<() => void> {
    return await listenEvent<WorktreeChangeEvent>('worktree-changed', cb)
  }
}
```

```rust
// src-tauri/src/features/worktree_management/presentation/commands.rs
use tauri::State;
use crate::state::AppState;
use crate::error::AppResult;
use crate::features::worktree_management::domain::WorktreeInfo;
