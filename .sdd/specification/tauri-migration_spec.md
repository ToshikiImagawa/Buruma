...
- **TypeScript 側**: 既存の `src/domain/` (移行後のパス) を真実の源とする
...
| invokeCommand<T> | `src/lib/invoke/commands.ts` に定義される TypeScript ラッパー関数。`invoke<T>()` を `try/catch` し `IPCResult<T>` 形式に変換する |
| listenEvent<T> | `src/lib/invoke/events.ts` に定義される TypeScript ラッパー関数。`listen<T>()` を呼び出し、UnlistenFn を返す |
