...
> ※ `git_reset` は basic-git-operations に登録されているが、hard リセットは不可逆操作であるため B-002 準拠の確認ダイアログが必要（ui-integration-advanced-git-operations で UI を定義）
...
| FR_301 | FR-001〜FR-005 + `git_stage` / `git_unstage` API | 対応済み |
| FR_301_01 | FR-001 + `git_stage` | 対応済み |
| FR_301_02 | FR-002 + `git_unstage` | 対応済み |
| FR_301_03 | FR-003（**Phase 2**） | 延期 |
| FR_301_04 | FR-004（**Phase 2**） | 延期 |
| FR_301_05 | FR-005 + `git_stage_all` / `git_unstage_all` | 対応済み |
| FR_302 | FR-006〜FR-010 + `git_commit` API | 対応済み |
| FR_303 | FR-011〜FR-014 + `git_push` API | 対応済み |
| FR_304 | FR-015〜FR-018 + `git_pull` / `git_fetch` API | 対応済み |
| FR_305 | FR-019〜FR-022 + `git_branch_create` / `git_branch_checkout` | 対応済み |
| FR_306 | FR-023〜FR-026 + `git_branch_delete` | 対応済み |
