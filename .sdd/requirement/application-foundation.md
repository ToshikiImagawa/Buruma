...
- IPC 通信基盤
    - 型安全な IPC チャネル定義
    - preload + contextBridge 経由の通信レイヤー
- エラーハンドリング
    - Git 操作エラーのユーザー通知
    - リトライ機能
    - エラー詳細表示
...
requirementDiagram
    ...
    requirement IPCFoundation {
        id: UR_004
        text: "メインプロセスとレンダラー間の安全で型安全な通信基盤を提供する"
        risk: high
        verifymethod: test
    }
    ...
    functionalRequirement IPCLayer {
        id: FR_604
        text: "preload + contextBridge 経由の型安全な IPC 通信レイヤーを提供する"
        risk: high
    ...
    designConstraint ElectronSecurity {
        id: DC_001
        text: "Electronセキュリティ制約: preload + contextBridge 経由の通信必須。レンダラーから Node.js API を直接使用しない"
        risk: high
        verifymethod: inspection
    }
    ...
    designConstraint DataPersistence {
        id: DC_002
        text: "設定・履歴データはローカルファイルシステムに永続化する（electron-store 等）"
        risk: low
        verifymethod: inspection
    }
    ...
    IPCFoundation - derives -> ElectronSecurity
...
Electron のセキュリティベストプラクティスに準拠し、メインプロセスとレンダラープロセス間の型安全な通信レイヤーを提供する。全機能の IPC 通信がこの基盤を利用する。
...
メインプロセスとレンダラープロセス間の通信を、preload + contextBridge パターンで実装する。全 IPC チャネルに対して型定義を提供し、型安全性を保証する。

**含まれる機能:**

- FR_604_01: contextBridge 経由の API 公開パターン
- FR_604_02: IPC チャネルの型定義（TypeScript）
- FR_604_03: リクエスト/レスポンス型の IPC ハンドラーパターン
- FR_604_04: イベントベースの IPC 通知パターン（main → renderer）
...
### DC_001: Electron セキュリティ制約

メインプロセスとレンダラープロセスの通信は preload + contextBridge を経由すること。レンダラーから Node.js API を直接使用しない。`nodeIntegration: false`, `contextIsolation: true` を維持する。
