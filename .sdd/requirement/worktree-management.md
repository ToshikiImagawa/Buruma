...
- ワークツリー作成
    - 既存ブランチを指定した作成
    - ローカル/リモートブランチ一覧からの選択
    - 新規ブランチを指定した作成
    - 作成先パスの指定
    - gitignore 対象ファイルのベースブランチからのシンボリックリンク
- ワークツリー削除
    - 安全確認ダイアログ付き削除
    - 未コミット変更がある場合の警告
    - 強制削除オプション
...
requirementDiagram
    ...
    functionalRequirement CreateWorktree {
        id: FR_102
        text: "新規ワークツリーを作成する（既存/新規ブランチ指定、ローカル/リモートブランチ選択、パス指定）"
        risk: high
        verifymethod: test
    }
    ...
    functionalRequirement DeleteWorktree {
        id: FR_103
        text: "既存ワークツリーを削除する（安全確認ダイアログ、未コミット変更警告、ローカルブランチ同時削除オプション付き）"
        risk: high
        verifymethod: test
    }

    functionalRequirement SymlinkIgnoredFiles {
        id: FR_106
        text: "ワークツリー作成時に設定ファイルに基づき、gitignore 対象ファイルをベースブランチのワークツリーからシンボリックリンクでコピーする"
        risk: medium
        verifymethod: test
    }
...
- FR_102_05: ローカル/リモートブランチ一覧からの選択UI（検索・フィルタリング付き）
...
- FR_103_05: ローカルブランチの同時削除オプション（チェックボックスで選択、`git branch -d` 相当）
...
### FR_106: gitignore 対象ファイルのシンボリックリンク

ワークツリー作成時に、設定ファイルで指定されたパターンに基づき、gitignore 対象のファイル/ディレクトリをベースブランチのワークツリーからシンボリックリンクとして新規ワークツリーにコピーする。ビルドキャッシュ、依存パッケージ（`node_modules`等）、IDE 設定ファイルなど、gitignore 対象だがワークツリー間で共有したいファイルを対象とする。

**含まれる機能:**

- FR_106_01: シンボリックリンク対象パターンの設定ファイル定義（アプリ設定またはリポジトリローカル設定）
- FR_106_02: ワークツリー作成時のベースブランチワークツリーからのシンボリックリンク自動作成
- FR_106_03: シンボリックリンク対象パターンの設定UI（追加・削除・一覧表示）
- FR_106_04: シンボリックリンク作成失敗時のエラーハンドリング（作成処理自体は続行）
...
| ベースブランチ | シンボリックリンクのコピー元となるワークツリーのブランチ。ワークツリー作成時に指定する |
...
| 推奨 (Should) | 5（UR_104, FR_105, FR_106, NFR_101, NFR_102） |
| 任意 (Could) | 0 |
