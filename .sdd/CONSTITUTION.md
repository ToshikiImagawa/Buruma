...
| ディレクトリ | プロセス | 役割 |
|:---|:---|:---|
| `src/main/` | メインプロセス | Node.js API、IPC ハンドラー、データ永続化。4層構成で feature を実装 |
| `src/renderer/` | レンダラープロセス | React UI、ViewModel、Hook。4層構成で feature を実装 |
| `src/shared/` | 共有 | プロセス間で共有する domain 型、IPC 型定義、DI ライブラリ、ユーティリティ |
| `src/preload/` | preload | contextBridge による API 公開のみ。ビジネスロジックを含まない |

- renderer と main はどちらも相手に依存しない（対等で独立）
- 両プロセスが使う型・ライブラリは `src/shared/` に配置する
...
- [ ] `src/renderer/` が `src/main/` を import していないか（逆も同様）
- [ ] プロセス間共有の型・ライブラリが `src/shared/` に配置されているか
...
- `src/renderer/` から `src/main/` のモジュールを import する

**準拠例**:

- preload で contextBridge.exposeInMainWorld を使用して API を公開
- IPC チャネルごとに引数・戻り値の型定義を `src/shared/types/` に定義
- Git 操作をメインプロセスの ipcMain.handle で処理
- domain 型を `src/shared/domain/` に配置し、両プロセスから参照
...
**原則**: すべてのサービス・ロジックの依存関係は VContainer（`src/shared/lib/di`）を通じて注入し、直接インスタンス化やグローバル参照を避ける
...
- feature 間の直接参照は禁止する
- feature 間で共有が必要な型・ロジックは `src/shared/lib/` に切り出す
...
```
src/renderer/ (React UI, Clean Architecture 4層)
      ↓ infrastructure 層（IPC クライアント）
src/preload/ (contextBridge API Bridge)
      ↓ ipcRenderer.invoke
src/main/ (Node.js, Clean Architecture 4層)
...
- `src/renderer/` と `src/main/` は互いに import しない（対等で独立）
- 両プロセスが使う型・ライブラリは `src/shared/` に配置する
- `src/preload/` は `src/shared/types/` のみ参照する（ビジネスロジックを含まない）
...
- サービス間の依存は VContainer（`src/shared/lib/di`）を通じて注入する。直接 `new` やグローバル参照による結合を避ける
