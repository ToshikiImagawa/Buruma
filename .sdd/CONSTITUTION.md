...
| `src/domain/`, `src/lib/` | TypeScript (共有) | domain 型、DI、Tauri invoke クライアント、ユーティリティ、IPC 型定義 |
...
- [ ] 両側が使う型は `src/domain/` に配置され、Rust 側 `src-tauri/src/domain/` と serde で整合しているか
...
- `src/lib/invoke/commands.ts` で `invokeCommand<T>(cmd, args)` ラッパーを通じて呼び出す
- Webview は `@tauri-apps/api/core` と `@tauri-apps/api/event` のみを import する
- IPC コマンドごとに引数・戻り値の型を `src/lib/ipc.ts` または `src/lib/invoke/tauri-api.ts` に定義する
- Git 操作を Rust 側の `#[tauri::command]` 関数で処理し、`tokio::process::Command` 経由で `git` CLI を呼び出す
- domain 型を `src/domain/` に配置し、`src-tauri/src/domain/` から serde 付き構造体として再定義する
...
- **Webview (TypeScript)**: `VContainer`（`src/lib/di`）+ `VContainerProvider`（React 統合）+ `useResolve()` Hook でコンテナを提供
...
| 層 | 配置場所 | 責務 | 許可される依存 |
|:---|:---|:---|:---|
| **domain** | `src/domain/` | エンティティ、値オブジェクト（境界間共有） | なし（純粋 TypeScript） |
...
- feature 間の直接参照は禁止する
- feature 間で共有が必要な型・ロジックは `src/domain/`・`src/lib/` または `src-tauri/src/` 直下（`src-tauri/src/git/` 等の横断モジュール）に切り出す
...
- [ ] UseCase が `src/lib/usecase/types.ts` の型を implements しているか（Webview）
...
- feature をまたいで domain 層を直接参照する（共有は `src/domain/`・`src/lib/` または `src-tauri/src/` 経由）
- React コンポーネント内で直接 `invoke()` を呼び出す
- ViewModel 内で `useState` や `useEffect` を使用する
- UseCase 内に BehaviorSubject 等の状態を保持する
- UseCase が `src/lib/usecase/types.ts` の型を implements していない（Webview）
...
**適用範囲**: `src/domain/`、`src/features/*/application/`

**検証方法**:

- [ ] `src/domain/` に React, `@tauri-apps/api`, OS API のインポートが存在しないか
...
- Tauri command には型安全なラッパー関数を `src/lib/invoke/` に定義する
...
- 両境界が使う型は `src/domain/`（TypeScript）に配置し、Rust 側は `src-tauri/src/domain/` で serde 付き構造体として再定義する
- `features/` 配下は 4 層の依存方向を厳守: domain ← application ← infrastructure / presentation
- `src/domain/` と `src-tauri/src/domain/` はフレームワーク非依存（`React`, `@tauri-apps/api`, `tauri::*`, `RxJS` 等をインポートしない）
...
- **A-001**: 「Electron プロセス分離」→「Tauri プロセス分離 / Rust-TypeScript 境界」に全面書き換え。ディレクトリ表を `src/` (Webview) + `src-tauri/` (Rust) 構成に刷新。検証項目から `nodeIntegration` / `contextIsolation` / Fuses を削除し、CSP / capabilities / 入力バリデーションに置換
...
- **A-005**: Pure TypeScript の適用範囲を Webview 層（`src/domain/`, `src/features/*/application/`）に限定
...
- **モジュール構成図**: `src/features/`、`src/domain|lib`、`src/components/`、`src-tauri/` 構成に更新（Option B: Tauri 標準構造）
