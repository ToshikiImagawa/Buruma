...
├── lib/                    # 共有ライブラリ（DI, hooks, IPC 型, UseCase/Service 型, コミットグラフレイアウト等）
...
├── lib.rs                  # Tauri ビルダー、全コマンド登録（78 コマンド）
...
- **コマンド定義**: `src/lib/ipc.ts` の `IPCCommandMap` で全 78 コマンドの引数・戻り値型を定義（フロント／Rust 側 `tauri::generate_handler!` と 1:1 対応）
- **イベント定義**: `src/lib/ipc.ts` の `IPCEventMap` で Tauri イベント型を定義（`error-notify`, `worktree-changed`, `git-progress`, `claude-output`, `claude-session-changed`, `claude-command-completed`, `claude-review-result`, `claude-explain-result`, `claude-conflict-resolved` の 9 種）
...
- **イベント受信**: `src/lib/invoke/events.ts` の `listenEventSync<T>()` を使用（呼び出し時に同期的に unsubscribe 関数を返し、内部で非同期 `listen()` を発行）
...
各 feature はフロントエンド（`src/features/{name}/`、kebab-case）とバックエンド（`src-tauri/src/features/{name}/`、snake_case）の両方にディレクトリを持つ（例: `application-foundation` ↔ `application_foundation`）。
...
├── __tests__/               # 統合テスト（任意、Vitest）
├── di-tokens.ts             # Token + UseCase 型エイリアス + re-export
└── di-config.ts             # VContainerConfig（useClass + deps パターン）
```

各層ディレクトリ内にも `__tests__/` を配置してユニットテストを書く（例: `application/__tests__/`, `presentation/__tests__/`）。
...
├── mod.rs                   # モジュール公開
├── domain.rs                # エンティティ、値オブジェクト（単一ファイル）
├── application/
│   ├── repositories.rs      # リポジトリ trait 定義
│   └── usecases.rs          # UseCase 実装
├── infrastructure/          # リポジトリ実装（Git CLI、ファイルシステム等）
└── presentation/
    └── commands.rs          # #[tauri::command] ハンドラー
```
...
- `OperatorUseCase<T>` — 同じ型の値を受け取って変換後の値を返す（`invoke(arg: T): T`）
- `PredicateUseCase<T>` — 真偽値を返す（`invoke(arg: T): boolean`）
- `ReadOnlyReactiveProperty<T>` — 現在値 + Observable の読み取り専用ラッパー（`value` + `asObservable()`）
